...
@Description: 
微信小程序 金典SATINE 日常签到领积分，积分可以兑换实物
使用教程：
 1.将Cookie脚本添加到本地
 2.打开小程序，进入打卡签到页面，若提示获取Cookie成功则可使用该脚本
 3.禁用Cookie脚本，防止不必要的mitm产生

【Loon】 :
*************************
[Script]
cron "30 0 7 * * *" script-path=https://raw.githubusercontent.com/Sliverkiss/helloworld/master/Study/jdsatine.js, timeout=300, tag=金典SATINE
...
// env.js 全局
const $ = new Env("金典SATINE");

//环境变量
const env_name = "jdSatine";//环境变量名字
...
function getCookie() {
    if ($request && $request.method != 'OPTIONS') {
        const env = $request.headers['accessToken']
        if (env) $.setdata(env, env_name);
			console.log(env);
